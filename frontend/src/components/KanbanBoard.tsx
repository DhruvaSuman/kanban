...
export const KanbanBoard = ({ board, onBoardChange }: KanbanBoardProps) => {
  ...
  const activeBoard = board ?? internalBoard;
  ...
  const updateBoard = (updater: (previous: BoardData) => BoardData) => {
    ...
    setInternalBoard((previous) => updater(previous));
  };
  ...
  const cardsById = useMemo(() => activeBoard.cards, [activeBoard.cards]);
          ...
          <section className="grid gap-6 lg:grid-cols-5">
            {activeBoard.columns.map((column) => (
              <KanbanColumn
                key={column.id}
                column={column}
                cards={column.cardIds.map((cardId) => activeBoard.cards[cardId])}
